...
function init() {
  ...
  try {
    const parsed = JSON.parse(window.localStorage.getItem(SOULSWAP))
...
export function Updater() {
  ...
  useEffect(() => {
    window.localStorage.setItem(SOULSWAP, JSON.stringify({ ...state, [LAST_SAVED]: Math.floor(Date.now() / 1000) }))
  })
