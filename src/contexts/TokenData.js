    ...
    let bulkResults = await Promise.all(
      current &&
        ...
        current?.data?.tokens.map(async (token) => {
          ...
          const currentLiquidityUSD = data?.liquidity * ethPrice * data?.derivedETH
          const oldLiquidityUSD = oneDayHistory?.liquidity * ethPriceOld * oneDayHistory?.derivedETH
...
const getIntervalTokenData = async (tokenAddress, startTime, interval = 3600, latestBlock) => {
  ...
  try {
    ...
    for (let i = 0; i < index; ++i) {
      if (values[i].priceUSD === 0 || isNaN(values[i].priceUSD)) {
        ++countZeros
      } else {
        price = values[i].priceUSD
        break
      }
    }

    for (let i = 0; i < countZeros; ++i) {
      values[i].priceUSD = price
    }
...
export function useTokenPriceData(tokenAddress, timeWindow, interval = 3600) {
  ...
  useEffect(() => {
    ...
    const startTime =
      timeWindow === timeframeOptions.ALL_TIME ? 1641016800 : currentTime.subtract(1, windowSize).startOf('hour').unix()
